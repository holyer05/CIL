...
- `experiment_base/` 已通过基础设施 smoke test，但这只证明环境、数据加载、复现和 manifest 路径可用；不能作为 A05/A03 的研究证据。
...
`协议已固化，实验底座 smoke 已通过，未运行 A05 诊断实验`
...
`协议已固化，实验底座 smoke 已通过，未运行 A03 诊断实验`
...
- 可以写：项目已具备可追溯的最小运行底座，`BASE-001` manifest 记录 clean commit、环境、配置快照、class order、运行时间和 GPU memory。
- 不能写：任何关于 prototype sufficiency、current-data proxy、forgetting 或 baseline 优劣的经验结论。
- 下一步写作素材只能来自 `SANITY-PLAN-001` 和后续正式诊断实验；底座 smoke 只进入复现性/实验设置说明，不进入 results。
