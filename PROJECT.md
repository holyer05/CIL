...
1. 现有结果有多少由第一任务表示覆盖和类顺序决定？
2. 单 prototype 或 Gaussian 统计是否真的足以代表旧类？
3. pseudo-feature 是否位于真实旧类流形，而不仅是能用于正则化分类器？
4. 平均准确率、forgetting、存储和隐私口径是否支持现有论文结论？

IDEA-001“可观测性驱动的漂移补偿”已被 PI 审计评为 `Weak Reject`，保持暂缓，不再作为默认主线。

## 当前阶段

- 阶段：Assumption Mining Mode；完成 20 项领域隐含假设审计，尚未选定论文主线，禁止方法编码。
...
`IDEA_POOL.md` 已记录 20 个跨论文、跨方法的隐含假设。当前优先级为：

1. `A01`：第一任务表示覆盖是否决定大部分 EFCIL 上限和方法排名；
2. `A05`：单 prototype 是否真的是旧类充分表示；
3. `A07`：pseudo-feature 是否“统计正确但流形错误”。

选择理由：

- 三者覆盖的论文和已实现方法最广，不依赖某个特定 drift estimator。
- 可以通过 oracle 诊断、冻结评测和分布检验直接证伪，不需要先发明算法。
- 一旦被证伪，会改变对现有方法增益、benchmark 公平性和论文贡献的解释。

在三项诊断完成前，不进入新方法设计；如果它们均未被证伪，再回到其余假设重新排序。
